...
// FIXIT: 1) нет смысла передавать указатель на count, т.к.вы не модифицируете эту переменную
// 2) не нужно писать комментарии там, где они не добавляют новой информации ... например, "ф-я очистки памяти" ...
// кажется, что действительно нужный комментарий - пояснение входных/выходных параметров ф-и split
...
void main () {
    ...
    printf("Введенная строка:\n%s ", str);					// проверка изменеий в введеной строке
